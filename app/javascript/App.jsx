        ...
        props.current_user.user_role === "common" ? (
          <Wrapper true_user={props.true_user}>
            <Switch>
              ...
              />
              <Route
                path={`/user_id=${props.currentUser.id}`}
                component={Profile}
              />
              <Route
                path={`/main_page/course_id=${props.currentCourse.id}`}
                component={CurrentCourse}
              />
              <Route
                path={`/course_id=${props.currentCourse.id}`}
                component={CoursePages}
              />
              <Route
                path={`/draft_course_id=${props.currentDraftCourse.id}`}
                component={CreateCourseTreePages}
              />
            </Switch>
          </Wrapper>
        ) : (
          <Switch>
            ...
            <Route
              exact
              path={`/catalog/impersonations`}
              component={Impersonations}
            />
          </Switch>
        )
      ) : (
