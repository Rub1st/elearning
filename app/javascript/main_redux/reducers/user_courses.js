...
const UserCourseReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_USER_COURSE: {
      notify(
        `Вы начали прохождение курса '${action.value.course.label}'!`,
        toast.info
      );
    ...
    case UPDATE_USER_COURSE: {
      ...
      if (
        action.value.progress == 100 &&
        action.value.correct > 90 &&
        action.value.mark == null
      ) {
        notify(
          `Примите наши поздравления! Вы успешно завершили курс '${action.value.course.label}'!`,
          toast.info
        );
        notify(
          `Ваш сертификат уже ждет вас в разделе 'Мои сертификаты' в профиле!`,
          toast.info
        );
        notify(
          `Пожалуйста, оцените этот курс! Это может работе и развитию данной платформы.`,
          toast.info
        );
      }
      ...
      if (
        ...
        notify(
          `К сожалению вам не удалось успешно завершить курс '${action.value.course.label}'!`,
          toast.info
        );
        notify(
          `Попробуйте снова! Нельзя сдаваться на пути к знаниям!`,
          toast.info
        );
        notify(
          `Пожалуйста, оцените этот курс! Это может работе и развитию данной платформы.`,
      ...
      return {
        ...state,
        userCourses: [
          ...state.userCourses.filter((el) => el.id !== action.value.id),
          action.value,
        ],
