...
const CourseListItem = (props) => {
  ...
  return (
    <div className="course-list-item">
      <div style={{ marginRight: "25px", marginTop: "12px" }}>{el.label}</div>
      ...
      {props.manager ? (
        <Tooltip title={t("Tooltip.8")}>
          <IconButton
            onClick={() =>
              props.put(props.newEl, "courses", updateCourseApproveStatus)
            }
          >
            <DeleteIcon />
          </IconButton>
      ...
      {props.manager &&
      props.reports.filter((el) => el.course.id === el.id) &&
      props.currentCourseId === el.id ? (
        !props.showReport ? (
