    ...
    <div className="home__position">
      <div className="home__top-line">
        ...
        <div className="home__main-user-info">
          ...
          <div className="home__main-info-item">
      ...
      <div className="home__second-line">
        <div className="home__secondary-user-info">
          {t("Profile.10")}{" "}
          {
            props.courses.filter((el) => el.author.id === props.currentUser.id)
              .length
          }
        </div>
        <div className="home__secondary-user-info">
          {t("Profile.11")}{" "}
          {
            props.userCourses.filter(
              (el) => el.user.id === props.currentUser.id && el.is_favorite
            ).length
          }
        </div>
        <div className="home__secondary-user-info">
        ...
        <div className="home__secondary-user-info">
          {t("Profile.14")}{" "}
          {
            props.comments.filter((el) => el.author.id === props.currentUser.id)
              .length
          }
        </div>
        <div className="home__secondary-user-info">
          {t("Profile.15")}{" "}
          {
            props.comments
              .map((el) => el.replies)
              .flat()
              .filter((el) => el.author.id === props.currentUser.id).length
          }
        </div>
      </div>
    </div>
