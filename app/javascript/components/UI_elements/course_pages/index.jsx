...
const CoursePages = (props) => {
  ...
  let selectedPages = props.pages.filter(el => el.course.id === props.currentCourse.id).sort((a,b) => a.order - b.order)

  const steps = selectedPages.map(el => el.title)

  let pageCount = selectedPages.length;

  const [slider, setSlider] = useState(props.currentUserCourse.current_page > pageCount ? 1 : props.currentUserCourse.current_page)
...
export default connect(
  ...
  dispatch => ({
    put: (obj, path) => dispatch(updateDataElement(obj, path, updateUserCourse))
  })
)(CoursePages);