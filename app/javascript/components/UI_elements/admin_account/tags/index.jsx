...
import { getData, postDataElement, updateDataElement } from '../../../../main_redux/actions/server_connections'
  ...
  return(
    <div>
      ...
      <TextField style={{marginLeft: '10px'}} value={tag} label={'create new tag'} variant="outlined" onChange={(e) => setTag(e.target.value)}/>
        <IconButton style={{marginLeft: '10px'}} onClick={() => {props.post({ name: tag}, 'admin/tags', createTag); setTag('')}}>
          <CreateOutlinedIcon/>
        </IconButton>
        <hr/>
    </div>
    <div>
      {
        ...
        {
          props.tags.map(el => <li key={el.id} className='tag-list-item'>
            <TagItem el={el}/>
          </li>)
        }
