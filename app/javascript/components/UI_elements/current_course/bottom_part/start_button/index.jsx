...
import {
  addUserCourse,
  updateUserCourse,
} from "../../../../../main_redux/actions/user_courses";
import Button from "@material-ui/core/Button";
import "./start_button.css";
import { useTranslation } from "react-i18next";

const StartButton = (props) => {
  let existed = props.userCourses.filter(
    (el) =>
      el.user.id === props.currentUser.id &&
      el.course.id === props.currentCourse.id
  );
  ...
  return (
    <div className="course__start-button-position">
      <Link
        to={`/course_id=${props.currentCourse.id}`}
        className={"course__start-link"}
        onClick={() => {
          !existed.length && props.addUserCourse(userCourse);
          !existed.length
            ? props.post(userCourseDB, "user_courses", updateUserCourse)
            : props.addUserCourse(existed[0]);
        }}
      >
        <Button
          className={"course__start-button"}
          variant="contained"
          color="primary"
        >
...
export default connect(
  ...
  (dispatch) => ({
    ...
    addUserCourse: (currentUserCourse) =>
      dispatch(addUserCourse(currentUserCourse)),
  })
)(StartButton);
