...
import { createPage, updatePage } from '../../../../../main_redux/actions/pages';
...
const CreateCourseTreePage = (props) => {
  ...
  let certainQuestions = props.questions.filter(e => e.page.id === props.currentDraftPage.id).filter(e => e.variants.filter(v => v.is_correct).length)
                    ...
                    <div className='question__position'>
                      ...
                      <Typography variant="body2" color="textSecondary" paragraph className='question__text'>{el.question_text}</Typography>
                          <div>
                            <ul>
                              {
                                el.variants.filter(e => e.question.id === el.id).map(e =>
                                  <li style={{listStyle: 'none', display: 'flex', justifyContent: 'space-between'}} key={e.id}>
                                    <Checkbox
