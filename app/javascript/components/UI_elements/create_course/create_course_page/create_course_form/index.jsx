...
const CreateCourseForm = (props) => {
  ...
  let managedOrganizations = props.organizations.filter(el => el.registered_members
    .filter(e => e.member_role === 'manager').map(e => e.user.id).includes(props.currentUser.id) )
  let accessTypes = [{id: 1, name: 'private'}, {id: 0, name: 'public'}, {id: 2, name: 'individual'}]
  ...
  let updatedCourse = {
    ...
    course: {
      organization_id: selectedOrganization.id === undefined ? null : selectedOrganization.id,
      access_type: accessType === 'public' ? 0 : accessType === 'private' ? 1 : 2,
    }
  }
          ...
          <div className='create-course-item'>
            <div className='create-course-item-top-row'>
              ...
              <div className='d-flex'>
                ...
                <Avatar className={classes.approve} alt={props.currentUser.login} src={image} />
              </div>
            </div>
            ...
            <div className='create-course-item'>
                ...
                <div className='create-course-item-top-row'>
                  <div className='create-course-select-list'>
                    ...
                    <Select
                      ...
                      value={organization}
                      onChange={(e) => { setOrganization(e.target.value); setPersons([]) }}
                    >
              ...
              <div className='create-course-select-list'>
                    ...
                    <Select
                      ...
                      {
                          accessTypes.filter(el => el.name !== 'private' || organization !== 'myself').map(el =>
                          <MenuItem value={el.name} key={el.id}>
                            {el.name}
                          </MenuItem>)
                      }
                      ...
                      <>
                        {
                          ...
                          <>
                            ...
                            {
                              selectedOrganization.registered_members.filter(e => e.user.login.toLowerCase().includes(userSearch.toLowerCase()) || !userSearch.length).map(el =>
                                   <ListItem key={el.id} button >
                                    <Checkbox color='primary' onChange={() => setPersons( persons.includes(el.user) ? persons.filter(e => e.id !== el.user.id) : [...persons, el.user])}/>
                                    <ListItemText>
                    ...
                    <ul className='create-course-tags-list'>
                  {
                    props.tags.filter(el => (showTakenTags ? tags.includes(el) : true)).map(el =>
                                <div key={el.id} className='create-course-tags-item'>
                                    ...
                                    icon={ tags.includes(el) ? <HighlightOffIcon className={classes.tagIcon}/> : <AddCircleOutlineIcon className={classes.tagIcon}/>}
                                    label={el.name}
                                    onClick={() => setTags( tags.includes(el) ? tags.filter(e => e.id !== el.id) : [...tags, el])}
             ...
             <div className='create-course-item-bottom-row'>
             <div>{slider + '/3'}</div>
              <Link to={`/draft_course_id=${props.currentDraftCourse.id}`} onClick={() => {
